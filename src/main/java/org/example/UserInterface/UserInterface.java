...
//        bikeRepository.save(ebike);
    ...
    private void manageRentals() {
        while (true) {
            ...
            System.out.println("1. Wypożycz Rower (WIP)");
            System.out.println("2. Zakończ wypożyczenie (NIECZYNNE)");
            System.out.println("3. Przeglądaj wszystkie wypożyczenia (NIECZYNNE)");
            System.out.println("4. Przeglądaj zakończone wypożyczenia (NIECZYNNE)");
            System.out.println("0. Powrót do menu głównego");
...
//                    listRentals();
    ...
    private void rentBike() {
        ...
        try {
            ...
            uuid = UUID.fromString(bikeId.toString());
            Bike bike = bikeDao.findById(uuid);
...
//        }
    ...
    private void removeBike() {
        ...
        List<Rental> currentRentals = rentalRepository.findByBikeId(uuid);// nie wiem jak zrobic zapytanie zeby sprawdzalo date. nie mam czasu.
        if (!currentRentals.isEmpty()) {
            for (Rental rental : currentRentals) {
                if (rental.getEndTime() != null) {
